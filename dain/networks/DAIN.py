...
from dain.loss_function import part_loss
import time
...
class DAIN(torch.nn.Module):
    def __init__(self,
        ...
        if self.training:
            self.flownets = PWCNet.__dict__['pwc_dc_net']("dain/PWCNet/pwc_net.pth.tar")
        else:
    ...
    def forward(self, input):
        ...
        '''
            STEP 1: pad and squeeze the input 
        '''
        w, h = input.size(4), input.size(3)
        padding_width, padding_height = 0, 0
        sh = 6
        if w != ((w >> sh) << sh):
            padding_width = (((w >> sh) + 1) << sh) - w
        if h != ((h >> sh) << sh):
            padding_height = (((h >> sh) + 1) << sh) - h
        ...
        if self.training == True:
            ...
            input_0, input_1, input_2 = paddingInput(input_0), paddingInput(input_1), paddingInput(input_2)
        else:
            assert input.size(0) ==2
            input_0,input_2 = torch.squeeze(input,dim=0)
            input_0, input_2 = paddingInput(input_0), paddingInput(input_2)
        ...
        # Unpad
        #print(cur_output.size(), cur_output_rectified.size())
        #print(type(cur_offset_output), type(cur_filter_output))
        #print(len(cur_offset_output), len(cur_filter_output))
        #print(cur_offset_output[0].size())
        cur_output = paddingOutput(cur_output)
        cur_output_rectified = paddingOutput(cur_output_rectified)
        cur_offset_output = [paddingOutput(_out) for _out in cur_offset_output]
        cur_filter_output = [paddingOutput(_out) for _out in cur_filter_output]
        if self.training == True:
            cur_input_1 = paddingOutput(cur_input_1)
        ...
        if self.training == True:
            ...
            return losses, offsets,filters,occlusions   , cur_output_rectified
        else:
...
class MetaDAIN(torch.nn.Module):
    def __init__(self,
                 channel = 3,
                 filter_size = 4,
                 timestep=0.5,
        ...
        self.ctxNet = S2D_models.__dict__['S2DF_3dense']()
        #self.ctxNet = S2D_models.__dict__['MetaS2DF_3dense']()
        self.ctx_ch = 3 * 64 + 3

        # self.rectifyNet = Resblock.__dict__['MultipleBasicBlock_4'](3 + 3 + 3 +2*1+ 2*2 +16*2+ 2 * self.ctx_ch, 128)
        self.rectifyNet = Resblock.__dict__['MetaMultipleBasicBlock_4'](3 + 3 + 3 +2*1+ 2*2 +16*2+ 2 * self.ctx_ch, 128)
        ...
        if resume:
            print('loading pretrained model from pretrained_models/dain_base.pth')
            self.load_state_dict(torch.load('pretrained_models/dain_base.pth'))
    ...
    def forward(self, input, params=None):
        ...
        if self.training == True:
            ...
            # return losses, loss_occlusion
            # return losses, offsets, filters, occlusions , cur_output_rectified
            pixel_loss, offset_loss, sym_loss = part_loss(losses, offsets, occlusions, [paddingOutput(input_0), paddingOutput(input_2)], epsilon=1e-8)
            total_loss = sum(x * y for x, y in zip([0.0, 1.0], pixel_loss))
            return total_loss, cur_output_rectified
        else:
            #cur_outputs = [cur_output,cur_output_rectified]
            #return cur_outputs, cur_offset_output, cur_filter_output
            return 0, cur_output_rectified
