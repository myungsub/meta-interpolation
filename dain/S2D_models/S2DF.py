...
__all__ = ['S2DF','S2DF_3dense','S2DF_3dense_nodilation',
           ...
           'MetaBasicBlock', 'MetaS2DF_3dense']
...
class MetaBasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, dilation = 1, stride=1, downsample=None):
        ...
        #self.conv1 = conv3x3(inplanes, planes,dilation, stride)
        self.conv1 = MetaConv2dLayer(inplanes, planes, kernel_size=3, stride=1, 
                                     padding=int(dilation*(3-1)/2), dilation=dilation, bias=False)
        # self.bn1 = nn.BatchNorm2d(planes)
        self.relu = nn.ReLU(inplace=True)
        #self.conv2 = conv3x3(planes, planes)
        self.conv2 = MetaConv2dLayer(planes, planes, kernel_size=3, stride=1, 
                                     padding=int(dilation*(3-1)/2), dilation=dilation, bias=False)
        # self.bn2 = nn.BatchNorm2d(planes)
        self.downsample = downsample
        self.stride = stride
...
class MetaS2DF(nn.Module):

    def __init__(self, block, num_blocks,dense = True, dilation=True):
        self.inplanes = 64
        super(MetaS2DF, self).__init__()
        self.dense = dense
        self.num_block = num_blocks
        assert(num_blocks>=1 and num_blocks<=4)
        self.block1 = MetaSequential(*[
            Conv2dLayer(3, 64, kernel_size=7, stride=1, padding=3, bias=False),
            nn.ReLU(inplace=True)
        ])
...
def S2DF_3dense():
    ...
    return model
def MetaS2DF_3dense():
    model = MetaS2DF(MetaBasicBlock,3,dense=True)
    return model
def S2DF_3last():
