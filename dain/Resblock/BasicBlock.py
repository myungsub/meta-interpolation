...
__all__ = ['MultipleBasicBlock','MultipleBasicBlock_4', 'MetaMultipleBasicBlock', 'MetaMultipleBasicBlock_4']
...
class MetaBasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, dilation = 1, stride=1, downsample=None):
        ...
        self.conv1 = MetaConv2dLayer(inplanes, planes, kernel_size=3, padding=1, stride=stride, use_bias=False) #conv3x3(inplanes, planes,dilation, stride)
        # self.bn1 = nn.BatchNorm2d(planes)
        self.relu = nn.ReLU(inplace=True)
        # self.conv2 = conv3x3(planes, planes)
        self.conv2 = MetaConv2dLayer(planes, planes, kernel_size=3, padding=1, stride=stride, use_bias=False)
        # self.bn2 = nn.BatchNorm2d(planes)
        self.downsample = downsample
        self.stride = stride
    ...
    def forward(self, x, params=None):
        ...
        if params is not None:
            ...
            out = self.conv1(x, params=param_dict['conv1'])
            # out = self.bn1(out)
            out = self.relu(out)
            ...
            if self.downsample is not None:
                residual = self.downsample(x, params=param_dict['downsample'])
        else:
            out = self.conv1(x)
            # out = self.bn1(out)
            out = self.relu(out)
...
class MetaMultipleBasicBlock(nn.Module):

    def __init__(self,input_feature,
                 block, num_blocks,
                 intermediate_feature = 64, dense = True):
        super(MetaMultipleBasicBlock, self).__init__()
        self.dense = dense
        self.num_block = num_blocks
        self.intermediate_feature = intermediate_feature

        self.block1= MetaSequential(*[
            MetaConv2dLayer(input_feature, intermediate_feature,
                      kernel_size=7, stride=1, padding=3, use_bias=True),
            nn.ReLU(inplace=True)
        ])
        ...
        self.block3 = block(intermediate_feature, intermediate_feature, dilation = 1) if num_blocks>=3 else None
        self.block4 = block(intermediate_feature, intermediate_feature, dilation = 1) if num_blocks>=4 else None
        self.block5 = MetaSequential(*[MetaConv2dLayer(intermediate_feature, 3 , 3, 1, 1)])
    ...
    def forward(self, x, params=None):
        ...
        if params is not None:
            param_dict = extract_top_level_dict(current_dict=params)
            x = self.block1(x, params=param_dict['block1'])
            x = self.block2(x, params=param_dict['block2']) if self.num_block>=2 else x
            x = self.block3(x, params=param_dict['block3']) if self.num_block>=3 else x
            x = self.block4(x, params=param_dict['block4']) if self.num_block== 4 else x
            x = self.block5(x, params=param_dict['block5'])
        else:
            x = self.block1(x)
            x = self.block2(x) if self.num_block>=2 else x
            x = self.block3(x) if self.num_block>=3 else x
...
def MetaMultipleBasicBlock_4(input_feature, intermediate_feature = 64):
    model = MetaMultipleBasicBlock(input_feature,
                                   MetaBasicBlock, 4,
                                   intermediate_feature)
    return model
