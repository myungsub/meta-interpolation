...
class VoxelFlow(nn.Module):
    ...
    def fix_batchnorm_parameters(self):
        for m in self.modules():
    ...
    def forward(self, x, syn_type='inter'):
        ...
        padW, padH = 0, 0
        if w != ((w >> shift) << shift):
            padW = (((w >> shift) + 1) << shift) - w
        if h != ((h >> shift) << shift):
            padH = (((h >> shift) + 1) << shift) - h
        ...
        output_1 = F.grid_sample(
            input[:, 0:3, :, :],
            torch.stack([coor_x_1, coor_y_1], dim=3),
            padding_mode='border')
        output_2 = F.grid_sample(
            input[:, 3:6, :, :],
        ...
        return paddingOutput(x)
        #return x


class MetaVoxelFlow(nn.Module):

    def __init__(self, config, resume=False):
        super(MetaVoxelFlow, self).__init__()
        self.config = config
        self.input_mean = [0.5 * 255, 0.5 * 255, 0.5 * 255]
        self.input_std = [0.5 * 255, 0.5 * 255, 0.5 * 255]

        self.syn_type = 'inter' #config.syn_type

        bn_param = dict(momentum=0.9997) #config.bn_param
        self.relu = nn.ReLU(inplace=True)
        self.pool = nn.MaxPool2d(kernel_size=2, stride=2)
        self.conv1 = MetaConv2dLayer(6, 64, kernel_size=5, stride=1, padding=2, use_bias=False)
        self.conv1_bn = BatchNorm2d(64, **bn_param)

        self.conv2 = MetaConv2dLayer(64, 128, kernel_size=5, stride=1, padding=2, use_bias=False)
        self.conv2_bn = BatchNorm2d(128, **bn_param)

        self.conv3 = MetaConv2dLayer(128, 256, kernel_size=3, stride=1, padding=1, use_bias=False)
        self.conv3_bn = BatchNorm2d(256, **bn_param)

        self.bottleneck = MetaConv2dLayer(256, 256, kernel_size=3, stride=1, padding=1, use_bias=False)
        self.bottleneck_bn = BatchNorm2d(256, **bn_param)

        self.deconv1 = MetaConv2dLayer(512, 256, kernel_size=3, stride=1, padding=1, use_bias=False)
        self.deconv1_bn = BatchNorm2d(256, **bn_param)

        self.deconv2 = MetaConv2dLayer(384, 128, kernel_size=5, stride=1, padding=2, use_bias=False)
        self.deconv2_bn = BatchNorm2d(128, **bn_param)

        self.deconv3 = MetaConv2dLayer(192, 64, kernel_size=5, stride=1, padding=2, use_bias=False)
        self.deconv3_bn = BatchNorm2d(64, **bn_param)

        self.conv4 = MetaConv2dLayer(64, 3, kernel_size=5, stride=1, padding=2)

        for m in self.modules():
            if isinstance(m, nn.Conv2d) or isinstance(m, MetaConv2dLayer):
                m.weight.data.normal_(0, 0.01)
                if m.bias is not None:
                    m.bias.data.zero_()
            elif isinstance(m, BatchNorm2d) or isinstance(m, MetaBatchNormLayer):
                m.weight.data.fill_(1)
                m.bias.data.zero_()

        if resume:
            #print('Loading model: pretrained_models/voxelflow_base.pth')
            #checkpoint = torch.load('pretrained_models/voxelflow_base.pth')
            print('Loading model: pretrained_models/voxelflow_ft.pth')
            checkpoint = torch.load('pretrained_models/voxelflow_ft.pth')
            self.load_state_dict(checkpoint['state_dict'])

        # We fix batchnorm parameters for all settings (train / val / test)
        self.fix_batchnorm_parameters()

        # Set optim policy parameters
        setattr(self.config, 'mult_conv_w', [1, 1])
        setattr(self.config, 'mult_conv_b', [2, 0])
        setattr(self.config, 'mult_bn', [1, 1])
        # self.config.mult_conv_w = [1, 1],  # lr, decay
        # self.config.mult_conv_b=[2, 0],  # lr, decay
        # self.config.mult_bn=[1, 1],  # lr, decay
    ...
    def train(self, mode=True):
        ...
        :return:
        """
        super(MetaVoxelFlow, self).train(mode)
        self.fix_batchnorm_parameters()
        '''if mode:
            convert_bn(self, self.config.bn_training, self.config.bn_parallel)
        else:
    ...
    def get_module_optim_policies(self, module, config, prefix):
        ...
        for m in module.modules():
            if isinstance(m, nn.Conv2d) or isinstance(m, MetaConv2dLayer):
                ...
                if len(ps) == 2:
                    bias.append(ps[1])
            elif isinstance(m, nn.BatchNorm2d) or isinstance(m, MetaBatchNormLayer):
                bn.extend(list(m.parameters()))
    ...
    def fix_batchnorm_parameters(self):
        for m in self.modules():
            if isinstance(m, nn.BatchNorm2d) or isinstance(m, MetaBatchNormLayer):
                m.eval()

    def forward(self, x0, x1, syn_type='inter', params=None, **kwargs):
        ...
        if params is not None:
            ...
            x = self.conv1(x, params=param_dict['conv1'])
            x = self.conv1_bn(x)
            conv1 = self.relu(x)

            x = self.pool(conv1)

            x = self.conv2(x, params=param_dict['conv2'])
            x = self.conv2_bn(x)
            conv2 = self.relu(x)

            x = self.pool(conv2)

            x = self.conv3(x, params=param_dict['conv3'])
            x = self.conv3_bn(x)
            conv3 = self.relu(x)

            x = self.pool(conv3)

            x = self.bottleneck(x, params=param_dict['bottleneck'])
            x = self.bottleneck_bn(x)
            x = self.relu(x)
            ...
            x = torch.cat([x, conv3], dim=1)
            x = self.deconv1(x, params=param_dict['deconv1'])
            x = self.deconv1_bn(x)
            x = self.relu(x)
            ...
            x = torch.cat([x, conv2], dim=1)
            x = self.deconv2(x, params=param_dict['deconv2'])
            x = self.deconv2_bn(x)
            x = self.relu(x)
            ...
            x = torch.cat([x, conv1], dim=1)
            x = self.deconv3(x, params=param_dict['deconv3'])
            x = self.deconv3_bn(x)
            x = self.relu(x)

            x = self.conv4(x, params=param_dict['conv4'])
            x = F.tanh(x)
